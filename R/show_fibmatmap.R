...
show_fibmatmap <- function(fibdata, yrsel, areasel,
  ...
  if(!isepchc & !isother){
    ...
    tomapseg <- dbasin[tomapsta %>%
      ...
      dplyr::inner_join(tomapsegcat, by = c('bay_segment' = 'grp')) %>%
      dplyr::mutate(

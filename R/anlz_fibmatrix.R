...
#' @concept show
#'
#' @return A \code{\link[dplyr]{tibble}} object with FIB summaries by year and station/bay segment (\code{grp}), including the estimated geometric mean of Enterococcus (marine) or E. coli (fresh) concentrations (\code{gmean}), the sample class (\code{class}, \code{"Marine"} or \code{"Fresh"}), a letter category based on the likelihood of exceeding the applicable threshold (\code{cat}), the count of samples over the lagged (\code{lagyr}) window exceeding that threshold (\code{nexceed}) and the total sample count over the same window (\code{nsamp}), and a continuous companion to \code{cat} (\code{exceed_rate}, see Details).  Results can be summarized by bay segment if \code{bay_segment} is not \code{NULL} and the input data is from \code{\link{read_importentero}}.
#'
...
#'
#' The \code{cat} letter grade is itself a discretized read on a single continuous quantity, also returned as \code{exceed_rate}: the one-sided 90\% upper confidence limit (Clopper-Pearson) on the true exceedance rate, given \code{nexceed} exceedances out of \code{nsamp} samples in the lagged window.  \code{cat} is assigned by testing that count against four fixed candidate rates (10/30/50/75\%) at a 10\% significance level and taking the lowest one still consistent with the data. \code{exceed_rate} is the closed-form value of the candidate rate at which that test statistic equals exactly 0.10 (\code{stats::qbeta(0.10, nexceed, nsamp - nexceed + 1)}), so it varies continuously with \code{nexceed}/\code{nsamp} instead of only at those four candidate rates, while remaining consistent with \code{cat} at its category boundaries and preserving the same sample-size-aware conservatism (small \code{nsamp} widens/raises the estimate). Lower \code{exceed_rate} is better (less likely the true exceedance rate is high).
#'
...
anlz_fibmatrix <- function(fibdata, yrrng = NULL, stas = NULL, bay_segment = NULL,
  ...
  dat <- fibdata %>%
    ...
    dplyr::mutate(
      ...
      exceed_rate = qbeta(0.10, sumgt, tot - sumgt + 1)
    ) %>%
  ...
  out <- dat %>%
    dplyr::select(yr, grp, class, gmean, Latitude, Longitude, cat = MWQA,
                  nexceed = sumgt, nsamp = tot, exceed_rate) %>%
    dplyr::mutate(

...
#' @param all logical indicating if only TBNI metrics are returned, otherwise all are calcualted
#'
...
anlz_tbnimet <- function(fimdata, all = TRUE){
  ...
  TaxaCountPrep <- fimdata %>%
    dplyr::group_by(Reference) %>%
    dplyr::mutate(TSTaxa = dplyr::case_when(Feeding_Cat == "TS" ~ 1, TRUE ~ 0),

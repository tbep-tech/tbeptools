...
attmap <- reactive({
  ...
  toplo <- epcdata %>% 
    anlz_avedatsite %>% 
    anlz_attainsite(thr = thrsel, trg = trginps, yrrng = yrsel) %>% 
    dplyr::mutate(
  ...
  mapview(toplo, fill = F, homebutton = F, popup = NULL, legend = F) %>% 
    ...
    addCircleMarkers(
      ...
      fill = TRUE,
      fillColor = ~cols,
      radius=~scales::rescale(val, to = c(1, 15)),
      weight = 1,
      fillOpacity = 1,
      label = ~paste0(epchc_station, ' (', trgtmet, ', ', round(val, 2), ')')
    ) %>% 
    addLegend("bottomright", colors = c('green', 'red'), labels = c('yes', 'no'), opacity = 1, title = "Target met?")

...
The `show_fibmap()` function creates a map of FIB sites and thresholds based on output from `anlz_fibmap()`.  The same arguments that apply to `anlz_fibmap()` also apply to `show_fibmap()` such that freshwater and marine stations categorized by relevant thresholds are plotted by a selected year, month, and area.  Unlike `anlz_fibmap()`, all arguments are required

```{r, out.width="100%"}
show_fibmap(fibdata, yrsel = 2022, mosel = 7, areasel = 'Hillsborough')
show_fibmap(fibdata, yrsel = 2021, mosel = 6, areasel = 'Alafia')
```

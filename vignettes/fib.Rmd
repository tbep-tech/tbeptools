...
The `import_readwqp()` function can be used to retrieve data from the USEPA Water Quality Portal for monitoring data from counties in and around the Tampa Bay watershed.  The function retrieves additional FIB data using `type = 'fib'` with the specified organization identified.  The data can be retrieved as follows and will typically take less than one minute to download. 

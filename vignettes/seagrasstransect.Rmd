...
There are two datasets included in tbeptools that show the transect locations that are actively monitored in Tampa Bay.  The `trnpts` dataset is a point object for the starting location of each transect and the `trnlns` dataset is a line object showing the approximate direction and length of each transect beginning at each point.  Each dataset also includes the `MonAgency` column that indicates with monitoring agency collects the data at each transect. 
...
The two datasets are `sf()` (simple features) object and are easily mapped with `mapview()` to view their locations.
...
The transect data can be downloaded from the Water Atlas using the `read_trnjsn()` function.  The only required argument for this function is `training` which indicates if you want to download training data or the complete dataset.  In the former case, a small dataset is downloaded that includes only data collected during an annual training event. These are used primarily to assess precision among different training crews. The data are downloaded as a JSON object, which can then be passed to the `anlz_trnsz()` function for formatting. 
...
Change the `training` argument to `FALSE` to download the entire transect database.  This may take a few seconds.  As before, the `anlz_trnjsn()` functions formats the JSON object that is returned by `read_trnjsn()`.

```{r}
# import entire transct dataset as JSON
trndat <- read_trnjsn(training = FALSE)

# format entire transect dataset
trndat <- anlz_trnjsn(trndat, training = FALSE)

...
If preferred, the matrix can also be returned in an HTML table that can be sorted and scrolled. Only the first ten rows are shown by defaul.  The default number of rows can be changed with the \code{nrows} argument.  Use a very large number to show all rows.

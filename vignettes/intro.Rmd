...
* `show_chlmatrix()`: Create a colorized table for chlorophyll annual exceedances by bay segment

* `show_boxplot()`: Plot monthly average chlorophyll data with FDEP criteria
...
* `show_thrplot()`: Plot annual water quality values and thresholds for a segment
...
After the package is loaded, you can view the help files for each function by typing a question mark followed by the function name, e.g., `?read_importwq`, on the console.  The help files provide a brief description of what each function does and the required farguments that are needed to run the function.
...
Similar information can be obtained for individual sites using `anlz_avedatsite()` and `anlz_attainsite()`.  The main difference is that a yes/no column `trgmet`is added that indicates only if the target was above or below the segment threshold for each site.
...
The condition of tidal creeks can be evaluated using the `anlz_tldcrk()` function.  This analysis uses an internal spatial data object (`tidalcreeks`) as the population of creeks to evaluate and raw water quality data from the Florida Department of Environmental Protection Impaired Waters Rule run 56.  The function evaluate different exceedances for total nitrogen in a given creek for the past ten years, with a lower score given to more frequent and severe exceedances. 

```{r}
tdlcrk <- anlz_tdlcrk(tidalcreeks, iwrraw, yr = 2018)
tdlcrk
```

As mentioned above, the analyze functions are used primarily within the visualization functions. They are mentioned here to provide a brief explanation of how to obtain the underlying data used in the plots.
...
The `show_thrplot()` function provides a more descriptive assessment of annual trends for a chosen bay segment relative to defined targets or thresholds. In this plot we show the annual averages across stations Old Tampa bay (`bay_segment = "OTB"`) for chlorophyll (`thr = "chla"`).  The red line shows annual trends and the horizontal blue lines indicate the regulator threshold for chlorophyll-a that is specific to Old Tampa Bay.  The dashed and dotted blue lines indicate +1 and +2 standard errors for the regulatory threshold.  The threshold and standard errors are considered when identifying the annual segment outcome for chlorophyll.
...
Similarly, the `show_boxplot()` function provides an assessment of seasonal changes in chlorophyll by bay segment.  The most recent year is highlighted in red by default. This allows a simple evaluation of how the most recent year compared to historical averages.  The FDEP criteria for the segment is shown in blue text and as the dotted line.  This corresponds to a "large" magnitude change of +2 standard errors above the bay segment threshold and is the same dotted line shown in `show_thrplot()`.    
...
<span style="color:#33FF3B">__Stay the Course__</span>: Continue planned projects. Report data via annual progress reports and Baywide Environmental Monitoring Report. 

<span style="color:#F9FF33">__Caution__</span>: Review monitoring data and nitrogen loading estimates. Begin/continue TAC and Management Board development of specific management recommendations.

<span style="color:#FF3333">__On Alert__</span>: Finalize development and implement appropriate management actions to get back on track.
...
Bay segment exceedances can also be viewed in a matrix using `show_chlmatrix`.  The thresholds for these values correspond to the Florida DEP criteria (or a large exceedance defined as +2 standard errors above the segment target).  
...
The tidal creek report card can also be viewed using `show_tdlcrk()`.

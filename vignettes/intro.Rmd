...
The `import_readwqp()` function can be used to retrieve data from the USEPA Water Quality Portal for monitoring data from Pinellas and Manatee County.  The function retrieves nutrient, chlorophyll, and secchi data for all available estuarine stations monitored by each county.  The data can be retrieved as follows and will typically take less than one minute to download. 

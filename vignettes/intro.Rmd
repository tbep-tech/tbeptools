...
The main functions are `show_matrix()` and `show_thrplot()`, the remainder are used internally in `show_matrix()`.
...
A different subset of years and selected year of interest can also be viewed by changing the `yrrng` and `yrcur` arguments.  Here we show 1980 compared to monthly averages for the last ten years. 

```{r, fig.height = 5, fig.width = 8}
show_boxplot(epcdata, bay_segment = "OTB", yrrng = c(2008, 2018), yrcur = 1980)
```

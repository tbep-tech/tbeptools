...
```{r, fig.cap = "Functions provided in tbeptools.", out.width = "80%", echo = F}
...
```{r, echo = F, out.width = '80%', fig.cap = "The help file for `read_importwq`."}
...
Here we use `anlz_avedat()` tosummarize the data by bay segment to estimate annual and monthly means for chlorophyll and light attenuation.  The output is a two-element list for the annual (`ann`) and monthly (`mos`) means by segment.
...
This output can then be further analyzed with `anlz_attain()` to determine if the bay segment targets are met in each year. 

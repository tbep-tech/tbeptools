...
* `show_colorizefld()`: Colorize a field with gt

* `show_colorizetbl()`: Colorize multiple fields with gt
...
The main functions are `show_boxplot()`, `show_chlmatrix()`, `show_matrix()`, and `show_thrplot()`. The remainder are used internally in `show_matrix()`.
...
Attainment outcomes for a single year can be returned in tabular format with `anlz_yrattain()`.  This table also shows segment averages for chlorophyll and light attenuation, including the associated targets. 
